...
export const CustomerDashboard: React.FC<CustomerDashboardProps> = ({
  ...
  const loadCustomerData = async () => {
    setIsLoading(true);
    try {
      const [bData, pData, prData, rData] = await Promise.all([
        apiRequest<Booking[]>('/api/bookings/my'),
        apiRequest<PrintOrder[]>('/api/print-orders/my'),
        apiRequest<PhotoProofing[]>('/api/photo-proofing/my'),
        apiRequest<Review[]>('/api/reviews/my')
      ]);
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'Confirmed':
        return <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-800">Confirmed</span>;
      case 'Awaiting Payment':
        return <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-100 text-amber-800">Awaiting Payment</span>;
      case 'Completed':
        return <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-purple-100 text-purple-800">Completed</span>;
      case 'Cancelled':
        ...
        <div className="space-y-4">
          {safeBookings.length === 0 ? (
            <div className="bg-white p-12 text-center rounded-3xl border border-stone-200 space-y-3">
              ...
              <h3 className="font-bold text-stone-800 text-sm">No reservations found</h3>
              <p className="text-xs text-stone-500">Explore Cainta photography studios and reserve your first session.</p>
              <button
                onClick={() => onNavigate('directory')}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {safeBookings.map(b => {
                ...
                return (
                  <div
                    key={b.id}
                    className="bg-white p-5 rounded-2xl border border-stone-200 shadow-xs hover:shadow-md transition-all flex flex-col justify-between space-y-4"
                  >
                    <div>
                      ...
                      <div className="mt-4 p-3 bg-stone-50 rounded-xl grid grid-cols-2 gap-2 text-xs">
                        ...
                        <div>
                          <span className="text-stone-400 text-[10px] uppercase font-semibold block">Balance Due</span>
                          <span className="font-bold text-amber-700">₱{b.remainingBalance.toLocaleString()}</span>
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        {b.remainingBalance > 0 && b.status !== 'Cancelled' && b.status !== 'Completed' && (
                          <button
                            ...
                            className="px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-xs font-bold flex items-center gap-1 shadow-xs"
                          >
                            <CreditCard className="w-3.5 h-3.5" /> Pay ₱{b.remainingBalance}
                          </button>
                        )}
